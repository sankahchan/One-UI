...
CMD = "cd /opt/one-ui && git fetch --all && git reset --hard origin/main && docker compose up -d --build backend"
    ...
    if pid == 0:
        ...
        while True:
            try:
                ...
                output.append(data)
                print(data, end="")
                if "UPDATE_SUCCESS" in data: # Customize as needed or just wait
                    break
            except OSError:
                break
