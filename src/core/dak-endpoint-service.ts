...
export class DakEndpointService {
    ...
    private async getOrCreateAccessKeyResponse(accessKeyName: string, serverId: number) {
        ...
        let accessKey = server.accessKeys.find((k: { name: string }) => k.name === accessKeyName);
    ...
    private selectRandomServerKey(keys: AccessKey[]): AccessKey {
        ...
        const serverGroups: AccessKey[][] = Array.from(grouped.values());
        const randomGroup: AccessKey[] = serverGroups[Math.floor(Math.random() * serverGroups.length)];
