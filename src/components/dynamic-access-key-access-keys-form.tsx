...
export default function DynamicAccessKeyAccessKeysForm({ dynamicAccessKey, servers }: Props) {
    ...
    const form = useForm<FormProps>({
        defaultValues: {
            accessKeys: dynamicAccessKey.accessKeys.map((k: { id: number }) => k.id.toString())
        }
    });
                            ...
                            {servers?.map((server) => (
                                <Card key={server.id} className="w-[360px]">
                                    ...
                                    <CardBody className="bg-content2">
                                        {server.accessKeys.map((accessKey: { id: number; name: string }) => (
                                    ...
                                    {server.tags?.length > 0 && (
                                        <CardFooter>
                                            <div className="flex gap-2 items-center flex-wrap">
                                                {server.tags.map((t: { tag: { id: number; name: string } }) => (
